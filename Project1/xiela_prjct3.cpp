...
#pragma omp parallel for// the #pragma goes here -- you figure out what it needs to look like:
		for (int i = 0; i < NUMCITIES; i++) // assign that city to belong to that capital
		{
			int capitalnumber = -1;
			float mindistance = 1.e+37;

			for (int k = 0; k < NUMCAPITALS; k++) // check distance to each of the capitals, check which state the city belongs to
			{
				float dist = Distance(i, k); // distance from numcapital to numcity
				if (dist < mindistance) 
				{					
					mindistance = dist;
...
#pragma omp critical
	...
	for (int k = 0; k < NUMCAPITALS; k++)
		...
		Capitals[k].name = Cities[capitalnumber].name.c_str();
